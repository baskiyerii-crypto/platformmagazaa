...
import { Plus, Trash2, KeyRound, UserPlus, ChevronDown, ChevronUp, Pencil, Save, X } from "lucide-react";
...
export function StoresManager() {
  ...
  async function createStoreWithUser(e: React.FormEvent) {
    ...
    const res = await fetch("/api/v1/admin/stores", {
      ...
      body: JSON.stringify({ name, address, active: true, username, password }),
    });
        ...
        <section className="panel-section">
          ...
          <form onSubmit={createStoreWithUser} className="grid gap-4 sm:grid-cols-2">
            <div className="space-y-1.5">
              ...
              <Input value={name} onChange={(e) => setName(e.target.value)} placeholder="Örn: Kadıköy Şubesi" required />
            </div>
            <div className="space-y-1.5">
            ...
            <div className="space-y-1.5">
              ...
              <Input value={username} onChange={(e) => setUsername(e.target.value)} placeholder="magaza_kullanici" required minLength={3} />
            </div>
            <div className="space-y-1.5">
              ...
              <div className="flex flex-col gap-4 md:flex-row md:items-start md:justify-between">
                <div>
                  ...
                  <p className="text-sm text-muted-foreground">
                    AVM: {store._count.avmEntries} · Açık Hava: {store._count.outdoorEntries} · Talep: {store._count.changeRequests}
                  </p>
                </div>
                ...
                {store.users.length === 0 ? (
                  ...
                  <div className="space-y-2">
                    {store.users.map((user) => (
                      <div key={user.id} className="flex items-center justify-between rounded-lg bg-secondary px-3 py-2 text-sm">
                        <span>{user.username}</span>
                        {isAdmin && (
                          <div className="flex gap-1">
                            <Button variant="ghost" size="sm" onClick={() => resetPassword(store.id, user.id)} title="Şifre sıfırla">
                              <KeyRound className="h-4 w-4" />
                            </Button>
                            <Button variant="ghost" size="sm" onClick={() => deleteUser(store.id, user.id)} title="Sil">
